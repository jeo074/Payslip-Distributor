...
# GLOBAL
# XPos = fpdf.enums.XPos
# YPos = fpdf.enums.YPos
                ...
                for j in range(0, len(return_pos)):
                    ...
                    if len(cells) > cell:
                        if 'formattedValue' in cells[cell]:
                            ...
                            # {sheetname: {param1: value1, param2: value2}...}
        return return_dict
...
def get_data(sheets, input_file):
    # cols = {sheetname: {param1: val1, param2: val2}, ...}
    vals_dict = {}
        ...
        if col_width == 'even':
            ...
            if isinstance(table_data, dict):
                ...
                for head, col in table_data.items():
                    ...
                    col_widths.append(longest + 20)  # add 4 for padding
...
def payslip_pdf(logo_file, title, id_no, branch, payroll_period, pay_table, deduct_table, employee_name, gross_pay,
    ...
    pdf.cell(200, 5, 'HUMAN PERSONA HR MANAGEMENT SERVICE, INC.', new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.cell(200, 5, 'No. 48 Panay Avenue, Quezon City, Metro Manila, 1103, Philippines', new_x=XPos.LMARGIN,
             new_y=YPos.NEXT, align='C')
    pdf.cell(200, 5, 'Office: (02) 372 2976', new_x=XPos.LEFT, new_y=YPos.NEXT, align='C')
    pdf.ln()
    pdf.set_font(style='B')
...
def main(start_date, end_date, request_queue, period_queue, exit_queue, enable_sending, payslip_type):
    ...
    try:
        ...
        logs_folder_id = '1FD7w0cs1eCoMTNd98wfTuQFd8nKbKr5a'
        tool_logs_folder_id = '1JvMUL5L573Wx70_4EkBdTVP3UPBlFJDN'
        PSID_sheet_id = '17OGwDQzSpZRsOajHCzEm98qGHQ3Wyavxga6rbZpBgnc'
                    ...
                    if enable_sending:
                        ...
                        if employee_email != '':
                            ...
                            body = ('Hi {},\n\nKindly see attached payslip for the payroll period {}.\n\n'
                                    ...
                                    'regards,\nHuman Persona Hr Management Service, Inc.').format(first_name, payroll_period)
    ...
    if file_queue == 0:
        request_queue.put(['log', ['No files found in "File" folder.', 'red']])
        add_logs(log_path + d.datetime.strftime(d.datetime.now(), '%Y-%m-%d'), 'No files found in "File" folder.')
...
# Check modules first
# modules = ['pandas', 'fpdf', 'tkinter', 'tkcalendar', 'customtkinter']
# pd, fpdf, tkinter, customtkinter = check_modules(modules)
