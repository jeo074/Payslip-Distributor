...
def goog_auth(SCOPES, CLIENT_FILE):
    creds = None
    if os.path.exists(r'C:\Users\jjdino\Python - Copy\Ops\token.json'):
        creds = Credentials.from_authorized_user_file(r'C:\Users\jjdino\Python - Copy\Ops\token.json', SCOPES)
    ...
    if not creds or not creds.valid:
        try:
            ...
            with open(r'C:\Users\jjdino\Python - Copy\Ops\token.json', 'w') as token:
...
def create_file(service, file_name, file_type, parent_folder_id=None, upload_file=None):
    ...
    try:
        ...
        file = service.files().create(body=file_metadata, supportsAllDrives=True, media_body=media, fields="id").execute()
        return file.get("id")
