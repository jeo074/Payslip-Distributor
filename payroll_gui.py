...
def on_closing():
    global exit_queue
    if t.is_alive():
        if tk.messagebox.askokcancel('Payslip Auto Distributor', 'The tool is still running.\n'
...
def run_main():
    ...
    if not t.is_alive():
        if tk.messagebox.askokcancel('Payslip Auto Distributor', 'Distribute payslip for payroll scope: {} - {}?'.format(start_date_str, end_date_str)):
            t.start()
        ...
        tk.messagebox.showwarning('Payslip Auto Distributor', 'A process is still currently running!')
...
def check_queue(request_queue):
    ...
    try:
        ...
        if not t.is_alive():
            ...
            tk.messagebox.showerror('Payslip Auto Distributor', 'An unexpected error has occured!')
        ...
        if to_do == 'add':
            ...
            update_progress(2, 1)
            t = threading.Thread(target=lambda: p.main(start_date, end_date, request_queue, period_queue, exit_queue, enable_sending, payslip_type))
            tk.messagebox.showinfo('Payslip Auto Distributor', 'Process has finished!\nKindly double check using logs.')
            gc.collect()
            return
        elif to_do == 'prompt':
            prompt_response = tk.messagebox.askyesno('Payslip Auto Distributor', params)
...
try:
    ...
    # prepare gui window
    gui.title("Payslip Auto Distributor")
    ...
    cal.grid(row=1, column=0, padx=(40, 10), pady=(4, 20), sticky="w")
    ...
    cal2.grid(row=1, column=1, padx=(0, 10), pady=(4, 20), sticky="nsew")
    update_calendar_appearance()
    ...
    period.grid(row=3, column=0, padx=(28, 0), pady=(5, 15), sticky="nsew", columnspan=2)
